...
Un \emph{reducer} è una \emph{funzione pura} che riceve come argomento un \verb|RAction| e ritorna una copia dello stato modificato. In kotlin il modo per realizzare una funzione \emph{reducer} è utilizzare una struttura \emph{switch} (in kotlin corrisponde ad una struttura \emph{when}) dove vengono definiti tanti casi quanto sono le \emph{actions} disponibili che vogliono essere utilizzate. Per le \emph{actions} definite precedentemente la funzione \emph{reducer} ha la seguente struttura:
